  ...
  <button className='button' onClick={toggleModal}>
   ...
   <div className='modal' id={user._id}>
        <div className='modal-background'>
          <div className='modal-card'>
            <header className='modal-card-head'>
              <p className='modal-card-title'>{user.name.first} {user.name.last}</p>
              <button
                className='delete'
                aria-label='close'
                onClick={toggleModal}
            ...
            <img className="avatar"   alt={`${user._id} img`} src={user.img} /> <br />
        ...
        <a href={user.contactLinks.other}> Other</a> <br />
         ...
         <span className="label">Current Location:</span>{user.location} 
         <span className='label'>Graduation Year: </span> {user.gradYear} 
         <span className='label'>Final Project:</span>{user.finalProject}
         <span className='label'>Employment Status: </span> {jobStatus}
           </h3>
           </div>
            </center>
            </section>
